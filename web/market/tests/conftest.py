...
def new_user(session):
    user = User(username='testname',email_address='test@test.com',password='testpass')
    session.add(user)
    session.commit()
    return user
...
def new_user2(session):
    user = User(username='testname2',email_address='test2@test.com',password='testpass')
    session.add(user)
    session.commit()
    return user
...
def new_item(session):
    item = Item(name='testitem',price=100,barcode=1212121,description='testitem')
    session.add(item)
    session.commit()
    return item
...
def new_item_over(session):
    item = Item(name='testitem2',price=6000,barcode=1212122,description='testitem2')
    session.add(item)
    session.commit()
    return item
...
def new_post(session,new_user):
    post = Post(title='testtitletest',post='somepostsomepost',author_id=new_user.id)
    session.add(post)
    session.commit()
    return post
...
def new_note(session,new_user):
    note = Note(title='testtitletest',notation='somenote_somenote',notation_id=new_user.id)
    session.add(note)
    session.commit()
    return note
...
def new_review(session,new_post,new_user):
    review = Review(review='testreviewonpost',reviewer_id=new_user.id,post_id=new_post.id)
    session.add(review)
    session.commit()
    return review
...
def new_review2(session,new_post,new_user):
    review = Review(review='testreviewonpost2',reviewer_id=new_user.id,post_id=new_post.id)
    session.add(review)
    session.commit()
    return review
...
def new_review_all(session,new_post,new_user2):
    review = Review(review='testreviewonpost3',reviewer_id=new_user2.id,post_id=new_post.id)
    session.add(review)
    session.commit()
    return review
